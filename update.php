...
if (isset($_POST['id'])){
  ...
  $firstname = $lastname = $birthday = $sex = $street = $barangay = $province = $userID = '';
...
if (isset($_POST['update'])) {
  if  ($con->updateUser($user_id, $firstname, $lastname, $birthday, $sex)) {
    ...
    $error = "An error occured while updating user address. Please try again.";{
...
//       }
//     }else{
//       $error = "An error occured while updating user information. Please try again.";
//   }
// }
  ...
  <form method="POST">
      ...
      <div class="card-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group col-md-6">
            ...
            Male</o>
            <option value="Female"<?php if($data['user_sex'] === 'Female') echo 'selected'; ?> >Female</option>>Female</option>
          </select>
          </div>
        </div>
      </div>
    ...
    <div class="row justify-content-center gx-0">
        ...
        <input type="hidden" name="id" value="<?php echo $data['user_id']?>">
            <input type="submit" name="update" class="btn btn-outline-primary btn-block mt-4">
